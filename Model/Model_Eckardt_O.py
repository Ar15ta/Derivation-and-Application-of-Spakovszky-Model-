# Model_Eckardt_O.py - Eckardt O 离心压气机动态系统模型 (无有叶扩压器)
import numpy as np
...
# Eckardt O 专用配置文件
DEFAULT_PARAMS_FILE = 'Eckardt_O_params.txt'

def compute_cc3_params(params_file=None):
    """加载并计算 Eckardt O 所有组件的平均流参数 (无有叶扩压器)
    ...
    # ==================== 从 STA 几何数据计算派生参数 ====================
    # STA 站位约定 (SPA 稳定性模型, 原点 x=0 在叶轮进口):
    #   STA1: 无 —— 上游以无限管道边界条件 IC_upstream_infinite 处理, 不设站位
    #   STA2: 叶轮进口 (特征半径 = r2rms)        (一维 station 2)
    #   STA3: 叶轮出口 / 无叶扩压器入口          (一维 station 4)
    #   STA4: 无叶扩压器出口 / 下游管道入口       (一维 station 5)
    #   STA5: 下游管道出口

    # 叶轮参数
    params['R1_R2'] = params['STA2_radial'] / params['STA3_radial']
    # AR_imp 从参数文件读取 (含密度比 ρ₃A₃/(ρ₂A₂), 由 Geo_Calculator 计算)
    ...
    # 调试输出计算结果
    print(f"[Eckardt O 参数计算] R1_R2 = {params['R1_R2']:.4f}")
    print(f"[Eckardt O 参数计算] AR_imp = {params['AR_imp']:.4f}  (含密度)")
    print(f"[Eckardt O 参数计算] lambda_imp = {params['lambda_imp']:.4f}")
    print(f"[Eckardt O 参数计算] tau_imp = {params['tau_imp']:.4f}")
    print(f"[Eckardt O 参数计算] Vx_plenum = {params['Vx_plenum']:.4f}")
...
def build_cc3_system_matrix(s, n, params=None, params_file=None):
    """
    构建 Eckardt O 系统传递矩阵 X_sys (上游系数 → 下游系数) (无有叶扩压器)
...
def compute_modal_shape(s_star, n, params, n_points=50, x_upstream_max=0.2):
    ...
    Vtheta_up = 0.0
    mu_up = params.get('mu_up', 1.0)
    r_in = params['r_impeller_exit']       # = STA3_radial = 1.0
    r_out = params['r_diffuser_exit']      # = STA4_radial = 1.69
    Q = params['Q']
    G = params['G']
